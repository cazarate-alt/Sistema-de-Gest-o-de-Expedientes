<?php
    ...
    function gerarCodigoAcao(PDO $pdo, string $nome, ?int $excludeId = null): string {
        ...
        while (true) {
            $sql = "SELECT COUNT(*) FROM `ACTION` WHERE ACTION_CODE = :c";
            $params = [':c'=>$codigo];
            if ($excludeId) { $sql .= " AND ACTION_ID <> :id"; $params[':id']=$excludeId; }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($action === 'save') {
            ...
            requirePermissionApi($pdo, 'ACTIONS', $isEdit ? 'UPDT' : 'CREA');
            ...
            if (!isset($errors['nome'])) {
                $sql = "SELECT COUNT(*) FROM `ACTION` WHERE ACTION_NAME = :n";
                $params = [':n'=>$nome]; if ($id) { $sql .= " AND ACTION_ID <> :id"; $params[':id']=$id; }
                $st = $pdo->prepare($sql); $st->execute($params);
                if ((int)$st->fetchColumn() > 0) $errors['nome'] = 'Já existe uma ação com este nome.';
            }
            if ($errors) { echo json_encode(['success'=>false,'errors'=>$errors]); exit; }
            try {
                if ($id) {
                    $st = $pdo->prepare("SELECT ACTION_NAME, ACTION_CODE FROM `ACTION` WHERE ACTION_ID=:id");
                    $st->execute([':id'=>$id]); $old = $st->fetch();
                    $code = ($old['ACTION_NAME'] !== $nome) ? gerarCodigoAcao($pdo, $nome, $id) : $old['ACTION_CODE'];
                    $pdo->prepare("UPDATE `ACTION` SET ACTION_CODE=:c, ACTION_NAME=:n, ACTION_DESCRIPTION=:d, ACTION_STATUS=:s WHERE ACTION_ID=:id")
                        ->execute([':c'=>$code,':n'=>$nome,':d'=>$desc,':s'=>$status,':id'=>$id]);
                    echo json_encode(['success'=>true,'message'=>'Ação atualizada com sucesso!']);
                } else {
                    $code = gerarCodigoAcao($pdo, $nome, null);
                    $pdo->prepare("INSERT INTO `ACTION` (ACTION_CODE, ACTION_NAME, ACTION_DESCRIPTION, ACTION_STATUS) VALUES (:c,:n,:d,:s)")
                        ->execute([':c'=>$code,':n'=>$nome,':d'=>$desc,':s'=>$status]);
                    echo json_encode(['success'=>true,'message'=>'Ação criada com sucesso!','id'=>(int)$pdo->lastInsertId()]);
                }
        ...
        if ($action === 'toggle') {
            requirePermissionApi($pdo, 'ACTIONS', 'UPDT');
            $id = intval($_POST['id'] ?? 0);
            $st = $pdo->prepare("SELECT ACTION_STATUS FROM `ACTION` WHERE ACTION_ID=:id");
            $st->execute([':id'=>$id]); $cur = $st->fetchColumn();
            if ($cur === false) { echo json_encode(['success'=>false,'message'=>'Ação não encontrada.']); exit; }
            $new = ((int)$cur === 1) ? 0 : 1;
            $pdo->prepare("UPDATE `ACTION` SET ACTION_STATUS=:s WHERE ACTION_ID=:id")->execute([':s'=>$new,':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>$new?'Ação ativada!':'Ação desativada!']);
            exit;
        }

        if ($action === 'delete') {
            requirePermissionApi($pdo, 'ACTIONS', 'DELE');
            $id = intval($_POST['id'] ?? 0);
            $st = $pdo->prepare("SELECT COUNT(*) FROM `PERMISSION` WHERE ACTION_ID=:id");
            $st->execute([':id'=>$id]);
            if ((int)$st->fetchColumn() > 0) { echo json_encode(['success'=>false,'message'=>'Existem permissões associadas.']); exit; }
            $pdo->prepare("DELETE FROM `ACTION` WHERE ACTION_ID=:id")->execute([':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>'Ação eliminada com sucesso!']);
            exit;
        }

        if ($action === 'stats') {
            $total = (int)$pdo->query("SELECT COUNT(*) FROM `ACTION`")->fetchColumn();
            $active = (int)$pdo->query("SELECT COUNT(*) FROM `ACTION` WHERE ACTION_STATUS=1")->fetchColumn();
            $inactive = (int)$pdo->query("SELECT COUNT(*) FROM `ACTION` WHERE ACTION_STATUS=0")->fetchColumn();
            $month = (int)$pdo->query("SELECT COUNT(*) FROM `ACTION` WHERE YEAR(ACTION_CREATEDAT)=YEAR(CURDATE()) AND MONTH(ACTION_CREATEDAT)=MONTH(CURDATE())")->fetchColumn();
    ...
    $acoes = $pdo->query("SELECT * FROM `ACTION` ORDER BY ACTION_ID DESC")->fetchAll();

    $pageTitle = 'Gestão de Ações';
    $activePage = 'actions.php';
...
?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div class="page-title mb-0" style="margin:0;">
                <h2><i class="fas fa-bolt" style="color:var(--accent-color);"></i> Gestão de Ações</h2>
                <div class="breadcrumb-custom"><a href="dashboard.php">Dashboard</a> <i class="fas fa-chevron-right mx-1" style="font-size:.7rem;"></i> <span>Ações</span></div>
            </div>
            <button class="btn-action" style="background:linear-gradient(135deg,#c0392b,#e74c3c);" onclick="window.open('export_pdf.php?tipo=actions','_blank')">
                <i class="fas fa-file-pdf"></i> Exportar PDF
            </button>
        </div>

        <div class="row mb-4">
            <div class="col-md-3 col-6 mb-3"><div class="stat-card"><div class="icon" style="background:rgba(52,152,219,.12);color:#3498db;"><i class="fas fa-bolt"></i></div><div class="stat-info"><div class="value" id="statTotal">—</div><div class="label">Total</div></div></div></div>
            <div class="col-md-3 col-6 mb-3"><div class="stat-card"><div class="icon" style="background:rgba(46,204,113,.12);color:#2ecc71;"><i class="fas fa-check-circle"></i></div><div class="stat-info"><div class="value" id="statActive">—</div><div class="label">Ativas</div></div></div></div>
            <div class="col-md-3 col-6 mb-3"><div class="stat-card"><div class="icon" style="background:rgba(231,76,60,.12);color:#e74c3c;"><i class="fas fa-times-circle"></i></div><div class="stat-info"><div class="value" id="statInactive">—</div><div class="label">Inativas</div></div></div></div>
        ...
        <div class="filter-card">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3"><label class="form-label">Estado</label><select class="form-select" id="filterStatus"><option value="">Todas</option><option value="1">Ativa</option><option value="0">Inativa</option></select></div>
                <div class="col-md-2"><label class="form-label">Ordenar</label><select class="form-select" id="filterOrderBy"><option value="ACTION_ID">ID</option><option value="ACTION_CODE">Código</option><option value="ACTION_NAME">Nome</option></select></div>
        ...
        <div class="main-card">
            <div class="main-card-header">
                <div class="title"><i class="fas fa-list-ul"></i><span>Lista de Ações</span><span class="badge bg-secondary ms-2" id="listCount" style="font-size:.75rem;">0</span></div>
                <button class="btn-action" onclick="openCreateModal()"><i class="fas fa-plus"></i> Nova Ação</button>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-custom">
                        ...
                        <tbody id="actionsTableBody">
                            <?php if (empty($acoes)): ?>
                                <tr><td colspan="6"><div class="empty-state"><i class="fas fa-inbox"></i><h6>Nenhuma ação</h6></div></td></tr>
                            <?php else: foreach ($acoes as $a):
                                $isActive = (int)$a['ACTION_STATUS'] === 1;
                            ?>
                                <tr id="row-<?= (int)$a['ACTION_ID'] ?>">
                                    <td><strong>#<?= (int)$a['ACTION_ID'] ?></strong></td>
                                    <td class="cell-code"><code style="background:#f1f5f9;padding:3px 8px;border-radius:4px;font-size:.8rem;"><?= htmlspecialchars($a['ACTION_CODE']) ?></code></td>
                                    <td class="cell-name"><strong><?= htmlspecialchars($a['ACTION_NAME']) ?></strong></td>
                                    <td class="cell-desc d-none d-md-table-cell" style="color:#7f8c8d;"><?= htmlspecialchars($a['ACTION_DESCRIPTION'] ?: '—') ?></td>
                                    <td class="cell-status">
                                        <?php if ($isActive): ?><span class="badge-status badge-active"><i class="fas fa-check-circle"></i> Ativa</span>
                                        <?php else: ?><span class="badge-status badge-inactive"><i class="fas fa-times-circle"></i> Inativa</span><?php endif; ?>
                                    </td>
                                    <td class="cell-actions"
                                        data-id="<?= (int)$a['ACTION_ID'] ?>"
                                        data-code="<?= htmlspecialchars($a['ACTION_CODE']) ?>"
                                        data-name="<?= htmlspecialchars($a['ACTION_NAME']) ?>"
                                        data-desc="<?= htmlspecialchars($a['ACTION_DESCRIPTION'] ?? '') ?>"
                                        data-status="<?= (int)$a['ACTION_STATUS'] ?>">
                                        <div class="action-links">
                                            <button class="btn-icon view" onclick="openViewModal(<?= (int)$a['ACTION_ID'] ?>)"><i class="fas fa-eye"></i></button>
                                            <button class="btn-icon toggle" onclick="openStatusModal(<?= (int)$a['ACTION_ID'] ?>)"><i class="fas fa-<?= $isActive?'toggle-on':'toggle-off' ?>"></i></button>
                                            <button class="btn-icon edit" onclick="openEditModal(<?= (int)$a['ACTION_ID'] ?>)"><i class="fas fa-edit"></i></button>
                                            <button class="btn-icon delete" onclick="openDeleteModal(<?= (int)$a['ACTION_ID'] ?>)"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
        ...
        <div class="modal fade" id="formModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header"><h5 class="modal-title" id="formModalTitle"><i class="fas fa-plus-circle"></i> Nova Ação</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <form id="actionForm" onsubmit="handleFormSubmit(event)" novalidate>
                        <div class="modal-body">
                            <input type="hidden" id="formActionId">
                            <div class="mb-3">
                            ...
                            <div class="mb-2">
                                ...
                                <div class="form-check form-switch">
                                    ...
                                    <label class="form-check-label" for="formStatus" style="font-size:.9rem;"><span id="formStatusLabel">Ativa</span></label>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <div class="modal-body text-center p-4">
                        ...
                        <h5 style="color:var(--primary-color);">Eliminar Ação</h5>
                        <p class="text-muted mb-0" style="font-size:.9rem;">Eliminar <strong id="deleteActionName" style="color:#e74c3c;"></strong>?</p>
                    </div>
                    <div class="modal-footer justify-content-center border-0 pb-4">
                        <button class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                document.getElementById('formStatus').addEventListener('change', function() { document.getElementById('formStatusLabel').textContent = this.checked ? 'Ativa' : 'Inativa'; });
                applyFilters();
            });
            ...
            function applyFilters(){
                ...
                if (orderBy) {
                    rows.sort((a,b) => {
                        ...
                        if (orderBy === 'ACTION_ID') return parseInt(B.id) - parseInt(A.id);
                        if (orderBy === 'ACTION_CODE') return A.code.localeCompare(B.code);
                        if (orderBy === 'ACTION_NAME') return A.name.localeCompare(B.name);
                        return 0;
                    });
            ...
            function resetFilters(){ document.getElementById('filterSearch').value=''; document.getElementById('filterStatus').value=''; document.getElementById('filterOrderBy').value='ACTION_ID'; applyFilters(); }

            function openViewModal(id){
                const row = document.getElementById('row-'+id); if (!row) return;
                const d = row.querySelector('.cell-actions').dataset;
                const st = d.status==='1' ? '<span class="badge-status badge-active"><i class="fas fa-check-circle"></i> Ativa</span>' : '<span class="badge-status badge-inactive"><i class="fas fa-times-circle"></i> Inativa</span>';
            ...
            function openCreateModal(){
                ...
                document.getElementById('formStatusLabel').textContent='Ativa';
                document.getElementById('formModalTitle').innerHTML='<i class="fas fa-plus-circle"></i> Nova Ação';
            ...
            function openEditModal(id){
                ...
                const d = row.querySelector('.cell-actions').dataset;
                clearAllErrors();
                document.getElementById('formActionId').value=d.id;
                ...
                document.getElementById('formStatusLabel').textContent = d.status==='1'?'Ativa':'Inativa';
                document.getElementById('formModalTitle').innerHTML='<i class="fas fa-edit"></i> Editar Ação';
                document.getElementById('formSubmitText').textContent='Atualizar';
                formModal.show();
            }
            function handleFormSubmit(e){
                e.preventDefault(); clearAllErrors();
                const id = document.getElementById('formActionId').value;
            ...
            function openStatusModal(id){
                ...
                pendingStatusId = id;
                document.getElementById('statusModalTitle').textContent = isActive?'Desativar Ação':'Ativar Ação';
            ...
            function openDeleteModal(id){
                const row = document.getElementById('row-'+id); if (!row) return;
                pendingDeleteId = id;
                document.getElementById('deleteActionName').textContent = row.querySelector('.cell-actions').dataset.name;
                deleteModal.show();
            }
