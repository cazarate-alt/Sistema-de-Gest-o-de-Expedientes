<?php
    ...
    function gerarCodigoTipo(PDO $pdo, string $nome, ?int $excludeId = null): string {
        ...
        while (true) {
            $sql = "SELECT COUNT(*) FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_CODE = :c";
            $params = [':c' => $codigo];
            if ($excludeId) { $sql .= " AND DOCUMENT_TYPE_ID <> :id"; $params[':id'] = $excludeId; }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($action === 'save') {
            ...
            requirePermissionApi($pdo, 'DOCTYPES', $isEdit ? 'UPDT' : 'CREA');
            ...
            if (!isset($errors['nome'])) {
                $sql = "SELECT COUNT(*) FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_NAME = :n";
                $params = [':n'=>$nome]; if ($id) { $sql .= " AND DOCUMENT_TYPE_ID <> :id"; $params[':id']=$id; }
                $st = $pdo->prepare($sql); $st->execute($params);
                if ((int)$st->fetchColumn() > 0) $errors['nome'] = 'Já existe um tipo com este nome.';
            }
            if ($errors) { echo json_encode(['success'=>false,'errors'=>$errors]); exit; }

            try {
                if ($id) {
                    $st = $pdo->prepare("SELECT DOCUMENT_TYPE_NAME, DOCUMENT_TYPE_CODE FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_ID=:id");
                    $st->execute([':id'=>$id]); $old = $st->fetch();
                    $code = ($old['DOCUMENT_TYPE_NAME'] !== $nome) ? gerarCodigoTipo($pdo, $nome, $id) : $old['DOCUMENT_TYPE_CODE'];
                    $pdo->prepare("UPDATE `DOCUMENT_TYPE` SET DOCUMENT_TYPE_CODE=:c, DOCUMENT_TYPE_NAME=:n, DOCUMENT_TYPE_DESCRIPTION=:d, DOCUMENT_TYPE_STATUS=:s WHERE DOCUMENT_TYPE_ID=:id")
                        ->execute([':c'=>$code,':n'=>$nome,':d'=>$desc,':s'=>$status,':id'=>$id]);
                    echo json_encode(['success'=>true,'message'=>'Tipo atualizado com sucesso!']);
                } else {
                    $code = gerarCodigoTipo($pdo, $nome, null);
                    $pdo->prepare("INSERT INTO `DOCUMENT_TYPE` (DOCUMENT_TYPE_CODE, DOCUMENT_TYPE_NAME, DOCUMENT_TYPE_DESCRIPTION, DOCUMENT_TYPE_STATUS) VALUES (:c,:n,:d,:s)")
                        ->execute([':c'=>$code,':n'=>$nome,':d'=>$desc,':s'=>$status]);
                    echo json_encode(['success'=>true,'message'=>'Tipo criado com sucesso!','id'=>(int)$pdo->lastInsertId()]);
                }
        ...
        if ($action === 'toggle') {
            requirePermissionApi($pdo, 'DOCTYPES', 'UPDT');
            $id = intval($_POST['id'] ?? 0);
            if ($id <= 0) { echo json_encode(['success'=>false,'message'=>'ID inválido.']); exit; }
            $st = $pdo->prepare("SELECT DOCUMENT_TYPE_STATUS FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_ID=:id");
            $st->execute([':id'=>$id]); $cur = $st->fetchColumn();
            if ($cur === false) { echo json_encode(['success'=>false,'message'=>'Tipo não encontrado.']); exit; }
            $new = ((int)$cur === 1) ? 0 : 1;
            $pdo->prepare("UPDATE `DOCUMENT_TYPE` SET DOCUMENT_TYPE_STATUS=:s WHERE DOCUMENT_TYPE_ID=:id")->execute([':s'=>$new,':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>$new?'Tipo ativado!':'Tipo desativado!']);
            exit;
        }

        if ($action === 'delete') {
            requirePermissionApi($pdo, 'DOCTYPES', 'DELE');
            $id = intval($_POST['id'] ?? 0);
            if ($id <= 0) { echo json_encode(['success'=>false,'message'=>'ID inválido.']); exit; }
            $st = $pdo->prepare("SELECT COUNT(*) FROM `DOCUMENT` WHERE DOCUMENT_TYPE_ID=:id");
            $st->execute([':id'=>$id]);
            if ((int)$st->fetchColumn() > 0) { echo json_encode(['success'=>false,'message'=>'Existem documentos associados.']); exit; }
            $pdo->prepare("DELETE FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_ID=:id")->execute([':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>'Tipo eliminado com sucesso!']);
            exit;
        }

        if ($action === 'stats') {
            $total = (int)$pdo->query("SELECT COUNT(*) FROM `DOCUMENT_TYPE`")->fetchColumn();
            $active = (int)$pdo->query("SELECT COUNT(*) FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_STATUS=1")->fetchColumn();
            $inactive = (int)$pdo->query("SELECT COUNT(*) FROM `DOCUMENT_TYPE` WHERE DOCUMENT_TYPE_STATUS=0")->fetchColumn();
            $inUse = (int)$pdo->query("SELECT COUNT(DISTINCT DOCUMENT_TYPE_ID) FROM `DOCUMENT`")->fetchColumn();
    ...
    $tipos = $pdo->query(
        "SELECT t.*, (SELECT COUNT(*) FROM `DOCUMENT` d WHERE d.DOCUMENT_TYPE_ID = t.DOCUMENT_TYPE_ID) AS TOTAL_DOCS
        FROM `DOCUMENT_TYPE` t ORDER BY t.DOCUMENT_TYPE_ID DESC"
    )->fetchAll();

    $pageTitle = 'Tipos de Documento';
    $activePage = 'document_types.php';
...
?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div class="page-title mb-0" style="margin:0;">
                <h2><i class="fas fa-file-signature" style="color:var(--accent-color);"></i> Tipos de Documento</h2>
                <div class="breadcrumb-custom"><a href="dashboard.php">Dashboard</a> <i class="fas fa-chevron-right mx-1" style="font-size:.7rem;"></i> <span>Tipos</span></div>
            </div>
            <button class="btn-action" style="background:linear-gradient(135deg,#c0392b,#e74c3c);" onclick="window.open('export_pdf.php?tipo=doctypes','_blank')">
                <i class="fas fa-file-pdf"></i> Exportar PDF
            </button>
        </div>

        <div class="row mb-4">
            <div class="col-md-3 col-6 mb-3"><div class="stat-card"><div class="icon" style="background:rgba(52,152,219,.12);color:#3498db;"><i class="fas fa-file-alt"></i></div><div class="stat-info"><div class="value" id="statTotal">—</div><div class="label">Total</div></div></div></div>
        ...
        <div class="filter-card">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-2"><label class="form-label">Ordenar</label><select class="form-select" id="filterOrderBy"><option value="DOCUMENT_TYPE_ID">ID</option><option value="DOCUMENT_TYPE_CODE">Código</option><option value="DOCUMENT_TYPE_NAME">Nome</option></select></div>
        ...
        <div class="main-card">
            <div class="main-card-header">
                <div class="title"><i class="fas fa-list-ul"></i><span>Lista de Tipos</span><span class="badge bg-secondary ms-2" id="listCount" style="font-size:.75rem;">0</span></div>
                <button class="btn-action" onclick="openCreateModal()"><i class="fas fa-plus"></i> Novo Tipo</button>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-custom">
                        <thead><tr>
                            ...
                            <th class="d-none d-md-table-cell">Descrição</th><th style="width:80px;">Docs</th>
                            <th style="width:110px;">Estado</th><th style="width:180px;">Ações</th>
                        </tr></thead>
                        <tbody id="typesTableBody">
                            <?php if (empty($tipos)): ?>
                                <tr><td colspan="7"><div class="empty-state"><i class="fas fa-inbox"></i><h6>Nenhum tipo</h6></div></td></tr>
                            <?php else: foreach ($tipos as $t):
                                $isActive = (int)$t['DOCUMENT_TYPE_STATUS'] === 1;
                            ?>
                                <tr id="row-<?= (int)$t['DOCUMENT_TYPE_ID'] ?>">
                                    <td><strong>#<?= (int)$t['DOCUMENT_TYPE_ID'] ?></strong></td>
                                    <td class="cell-code"><code style="background:#f1f5f9;padding:3px 8px;border-radius:4px;font-size:.8rem;"><?= htmlspecialchars($t['DOCUMENT_TYPE_CODE']) ?></code></td>
                                    <td class="cell-name"><strong><?= htmlspecialchars($t['DOCUMENT_TYPE_NAME']) ?></strong></td>
                                    <td class="cell-desc d-none d-md-table-cell" style="color:#7f8c8d;"><?= htmlspecialchars($t['DOCUMENT_TYPE_DESCRIPTION'] ?: '—') ?></td>
                                    <td><span class="badge bg-info"><?= (int)$t['TOTAL_DOCS'] ?></span></td>
                                    ...
                                    <td class="cell-actions"
                                        data-id="<?= (int)$t['DOCUMENT_TYPE_ID'] ?>"
                                        data-code="<?= htmlspecialchars($t['DOCUMENT_TYPE_CODE']) ?>"
                                        data-name="<?= htmlspecialchars($t['DOCUMENT_TYPE_NAME']) ?>"
                                        data-desc="<?= htmlspecialchars($t['DOCUMENT_TYPE_DESCRIPTION'] ?? '') ?>"
                                        data-status="<?= (int)$t['DOCUMENT_TYPE_STATUS'] ?>"
                                        data-docs="<?= (int)$t['TOTAL_DOCS'] ?>">
                                        <div class="action-links">
                                            <button class="btn-icon view" onclick="openViewModal(<?= (int)$t['DOCUMENT_TYPE_ID'] ?>)"><i class="fas fa-eye"></i></button>
                                            <button class="btn-icon toggle" onclick="openStatusModal(<?= (int)$t['DOCUMENT_TYPE_ID'] ?>)"><i class="fas fa-<?= $isActive?'toggle-on':'toggle-off' ?>"></i></button>
                                            <button class="btn-icon edit" onclick="openEditModal(<?= (int)$t['DOCUMENT_TYPE_ID'] ?>)"><i class="fas fa-edit"></i></button>
                                            <button class="btn-icon delete" onclick="openDeleteModal(<?= (int)$t['DOCUMENT_TYPE_ID'] ?>)" <?= $t['TOTAL_DOCS']>0?'disabled':'' ?>><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
        ...
        <div class="modal fade" id="formModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header"><h5 class="modal-title" id="formModalTitle"><i class="fas fa-plus-circle"></i> Novo Tipo</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <form id="typeForm" onsubmit="handleFormSubmit(event)" novalidate>
            ...
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <div class="modal-body text-center p-4">
                        ...
                        <h5 style="color:var(--primary-color);">Eliminar Tipo</h5>
                        <p class="text-muted mb-0" style="font-size:.9rem;">Eliminar <strong id="deleteTypeName" style="color:#e74c3c;"></strong>?</p>
                    </div>
                    <div class="modal-footer justify-content-center border-0 pb-4">
                        <button class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
        ...
        <script>
            const API_URL = 'document_types.php';
            ...
            function showFieldError(field, msg){ const id = field==='nome'?'errorName':null; if (!id) return; const el = document.getElementById(id); if (el) { el.querySelector('span').textContent = msg; el.classList.add('show'); } }
            function clearFieldError(field){ const id = field==='nome'?'errorName':null; if (id && document.getElementById(id)) document.getElementById(id).classList.remove('show'); }
            function clearAllErrors(){ clearFieldError('nome'); }
            ...
            function applyFilters(){
                ...
                if (orderBy) {
                    rows.sort((a,b) => {
                        ...
                        if (orderBy === 'DOCUMENT_TYPE_ID') return parseInt(B.id) - parseInt(A.id);
                        if (orderBy === 'DOCUMENT_TYPE_CODE') return A.code.localeCompare(B.code);
                        if (orderBy === 'DOCUMENT_TYPE_NAME') return A.name.localeCompare(B.name);
                        return 0;
                    });
            ...
            function resetFilters(){ document.getElementById('filterSearch').value=''; document.getElementById('filterStatus').value=''; document.getElementById('filterOrderBy').value='DOCUMENT_TYPE_ID'; applyFilters(); }
            ...
            function openCreateModal(){
                ...
                document.getElementById('formModalTitle').innerHTML='<i class="fas fa-plus-circle"></i> Novo Tipo';
            ...
            function openEditModal(id){
                ...
                const d = row.querySelector('.cell-actions').dataset;
                clearAllErrors();
                document.getElementById('formTypeId').value=d.id;
                ...
                document.getElementById('formModalTitle').innerHTML='<i class="fas fa-edit"></i> Editar Tipo';
                document.getElementById('formSubmitText').textContent='Atualizar';
                formModal.show();
            }
            function handleFormSubmit(e){
                e.preventDefault(); clearAllErrors();
                const id = document.getElementById('formTypeId').value;
            ...
            function openStatusModal(id){
                ...
                pendingStatusId = id;
                document.getElementById('statusModalTitle').textContent = isActive?'Desativar Tipo':'Ativar Tipo';
            ...
            function openDeleteModal(id){
                ...
                const d = row.querySelector('.cell-actions').dataset;
                pendingDeleteId = id;
                document.getElementById('deleteTypeName').textContent = d.name;
                deleteModal.show();
            }
