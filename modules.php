<?php
    ...
    function moduleStatusToInt($value): int {
    ...
    function gerarCodigoModulo(PDO $pdo, string $nome, ?int $excludeId = null): string {
        ...
        while (true) {
            $sql = "SELECT COUNT(*) FROM `MODULE` WHERE MODULE_CODE = :c";
            $params = [':c'=>$codigo];
            if ($excludeId) { $sql .= " AND MODULE_ID <> :id"; $params[':id']=$excludeId; }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
        ...
        if ($action === 'save') {
            ...
            requirePermissionApi($pdo, 'MODULES', $isEdit ? 'UPDT' : 'CREA');
            ...
            if (!isset($errors['nome'])) {
                $sql = "SELECT COUNT(*) FROM `MODULE` WHERE MODULE_NAME = :n";
                $params = [':n'=>$nome]; if ($id) { $sql .= " AND MODULE_ID <> :id"; $params[':id']=$id; }
                $st = $pdo->prepare($sql); $st->execute($params);
                if ((int)$st->fetchColumn() > 0) $errors['nome'] = 'Já existe um módulo com este nome.';
            }
            if ($errors) { echo json_encode(['success'=>false,'errors'=>$errors]); exit; }
            try {
                if ($id) {
                    $st = $pdo->prepare("SELECT MODULE_NAME, MODULE_CODE FROM `MODULE` WHERE MODULE_ID=:id");
                    $st->execute([':id'=>$id]); $old = $st->fetch();
                    $code = ($old['MODULE_NAME'] !== $nome) ? gerarCodigoModulo($pdo, $nome, $id) : $old['MODULE_CODE'];
                    $pdo->prepare("UPDATE `MODULE` SET MODULE_CODE=:c, MODULE_NAME=:n, MODULE_DESCRIPTION=:d, MODULE_STATUS=:s WHERE MODULE_ID=:id")
                        ->execute([':c'=>$code,':n'=>$nome,':d'=>$desc,':s'=>$status,':id'=>$id]);
                    echo json_encode(['success'=>true,'message'=>'Módulo atualizado com sucesso!']);
                } else {
                    $code = gerarCodigoModulo($pdo, $nome, null);
                    $pdo->prepare("INSERT INTO `MODULE` (MODULE_CODE, MODULE_NAME, MODULE_DESCRIPTION, MODULE_STATUS) VALUES (:c,:n,:d,:s)")
                        ->execute([':c'=>$code,':n'=>$nome,':d'=>$desc,':s'=>$status]);
                    echo json_encode(['success'=>true,'message'=>'Módulo criado com sucesso!','id'=>(int)$pdo->lastInsertId()]);
                }
        ...
        if ($action === 'toggle') {
            requirePermissionApi($pdo, 'MODULES', 'UPDT');
            $id = intval($_POST['id'] ?? 0);
            $st = $pdo->prepare("SELECT MODULE_STATUS FROM `MODULE` WHERE MODULE_ID=:id");
            $st->execute([':id'=>$id]); $cur = $st->fetchColumn();
            if ($cur === false) { echo json_encode(['success'=>false,'message'=>'Módulo não encontrado.']); exit; }
            $curInt = moduleStatusToInt($cur);
            $new = ($curInt === 1) ? 0 : 1;
            $pdo->prepare("UPDATE `MODULE` SET MODULE_STATUS=:s WHERE MODULE_ID=:id")->execute([':s'=>$new,':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>$new?'Módulo ativado!':'Módulo desativado!']);
            exit;
        }

        if ($action === 'delete') {
            requirePermissionApi($pdo, 'MODULES', 'DELE');
            $id = intval($_POST['id'] ?? 0);
            $st = $pdo->prepare("SELECT COUNT(*) FROM `PERMISSION` WHERE MODULE_ID=:id");
            $st->execute([':id'=>$id]);
            if ((int)$st->fetchColumn() > 0) { echo json_encode(['success'=>false,'message'=>'Existem permissões associadas.']); exit; }
            $pdo->prepare("DELETE FROM `MODULE` WHERE MODULE_ID=:id")->execute([':id'=>$id]);
            echo json_encode(['success'=>true,'message'=>'Módulo eliminado com sucesso!']);
            exit;
        }

        if ($action === 'stats') {
            $total = (int)$pdo->query("SELECT COUNT(*) FROM `MODULE`")->fetchColumn();
            $all = $pdo->query("SELECT MODULE_STATUS FROM `MODULE`")->fetchAll(PDO::FETCH_COLUMN);
            $active = 0; $inactive = 0;
            foreach ($all as $s) { moduleStatusToInt($s) === 1 ? $active++ : $inactive++; }
            $month = (int)$pdo->query("SELECT COUNT(*) FROM `MODULE` WHERE YEAR(MODULE_CREATEDAT)=YEAR(CURDATE()) AND MONTH(MODULE_CREATEDAT)=MONTH(CURDATE())")->fetchColumn();
    ...
    $modulos = $pdo->query("SELECT * FROM `MODULE` ORDER BY MODULE_ID DESC")->fetchAll();

    $pageTitle = 'Gestão de Módulos';
...
?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
            <div class="page-title mb-0" style="margin:0;">
                <h2><i class="fas fa-cubes" style="color:var(--accent-color);"></i> Gestão de Módulos</h2>
                <div class="breadcrumb-custom"><a href="dashboard.php">Dashboard</a> <i class="fas fa-chevron-right mx-1" style="font-size:.7rem;"></i> <span>Módulos</span></div>
            </div>
            <button class="btn-action" style="background:linear-gradient(135deg,#c0392b,#e74c3c);" onclick="window.open('export_pdf.php?tipo=modules','_blank')">
                <i class="fas fa-file-pdf"></i> Exportar PDF
            </button>
        </div>

        <div class="row mb-4">
            <div class="col-md-3 col-6 mb-3"><div class="stat-card"><div class="icon" style="background:rgba(52,152,219,.12);color:#3498db;"><i class="fas fa-cubes"></i></div><div class="stat-info"><div class="value" id="statTotal">—</div><div class="label">Total</div></div></div></div>
        ...
        <div class="filter-card">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-2"><label class="form-label">Ordenar</label><select class="form-select" id="filterOrderBy"><option value="MODULE_ID">ID</option><option value="MODULE_CODE">Código</option><option value="MODULE_NAME">Nome</option></select></div>
        ...
        <div class="main-card">
            <div class="main-card-header">
                <div class="title"><i class="fas fa-list-ul"></i><span>Lista de Módulos</span><span class="badge bg-secondary ms-2" id="listCount" style="font-size:.75rem;">0</span></div>
                <button class="btn-action" onclick="openCreateModal()"><i class="fas fa-plus"></i> Novo Módulo</button>
            </div>
                    ...
                    <table class="table table-custom">
                        ...
                        <tbody id="modulesTableBody">
                            <?php if (empty($modulos)): ?>
                                <tr><td colspan="6"><div class="empty-state"><i class="fas fa-inbox"></i><h6>Nenhum módulo</h6></div></td></tr>
                            <?php else: foreach ($modulos as $m):
                                $statusInt = moduleStatusToInt($m['MODULE_STATUS']);
                                $isActive = $statusInt === 1;
                            ?>
                                <tr id="row-<?= (int)$m['MODULE_ID'] ?>">
                                    <td><strong>#<?= (int)$m['MODULE_ID'] ?></strong></td>
                                    <td class="cell-code"><code style="background:#f1f5f9;padding:3px 8px;border-radius:4px;font-size:.8rem;"><?= htmlspecialchars($m['MODULE_CODE']) ?></code></td>
                                    <td class="cell-name"><strong><?= htmlspecialchars($m['MODULE_NAME']) ?></strong></td>
                                    <td class="cell-desc d-none d-md-table-cell" style="color:#7f8c8d;"><?= htmlspecialchars($m['MODULE_DESCRIPTION'] ?: '—') ?></td>
                                    ...
                                    <td class="cell-actions"
                                        data-id="<?= (int)$m['MODULE_ID'] ?>"
                                        data-code="<?= htmlspecialchars($m['MODULE_CODE']) ?>"
                                        data-name="<?= htmlspecialchars($m['MODULE_NAME']) ?>"
                                        data-desc="<?= htmlspecialchars($m['MODULE_DESCRIPTION'] ?? '') ?>"
                                        data-status="<?= $statusInt ?>">
                                        <div class="action-links">
                                            <button class="btn-icon view" onclick="openViewModal(<?= (int)$m['MODULE_ID'] ?>)"><i class="fas fa-eye"></i></button>
                                            <button class="btn-icon toggle" onclick="openStatusModal(<?= (int)$m['MODULE_ID'] ?>)"><i class="fas fa-<?= $isActive?'toggle-on':'toggle-off' ?>"></i></button>
                                            <button class="btn-icon edit" onclick="openEditModal(<?= (int)$m['MODULE_ID'] ?>)"><i class="fas fa-edit"></i></button>
                                            <button class="btn-icon delete" onclick="openDeleteModal(<?= (int)$m['MODULE_ID'] ?>)"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </td>
                                </tr>
        ...
        <div class="modal fade" id="formModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header"><h5 class="modal-title" id="formModalTitle"><i class="fas fa-plus-circle"></i> Novo Módulo</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                    <form id="moduleForm" onsubmit="handleFormSubmit(event)" novalidate>
            ...
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <div class="modal-body text-center p-4">
                        ...
                        <h5 style="color:var(--primary-color);">Eliminar Módulo</h5>
                        <p class="text-muted mb-0" style="font-size:.9rem;">Eliminar <strong id="deleteModuleName" style="color:#e74c3c;"></strong>?</p>
                    </div>
                    <div class="modal-footer justify-content-center border-0 pb-4">
                        <button class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
        ...
        <script>
            ...
            function applyFilters(){
                ...
                if (orderBy) {
                    rows.sort((a,b) => {
                        ...
                        if (orderBy === 'MODULE_ID') return parseInt(B.id) - parseInt(A.id);
                        if (orderBy === 'MODULE_CODE') return A.code.localeCompare(B.code);
                        if (orderBy === 'MODULE_NAME') return A.name.localeCompare(B.name);
                        return 0;
                    });
            ...
            function resetFilters(){ document.getElementById('filterSearch').value=''; document.getElementById('filterStatus').value=''; document.getElementById('filterOrderBy').value='MODULE_ID'; applyFilters(); }
            ...
            function openCreateModal(){
                ...
                document.getElementById('formModalTitle').innerHTML='<i class="fas fa-plus-circle"></i> Novo Módulo';
            ...
            function openEditModal(id){
                ...
                const d = row.querySelector('.cell-actions').dataset;
                clearAllErrors();
                document.getElementById('formModuleId').value=d.id;
                ...
                document.getElementById('formModalTitle').innerHTML='<i class="fas fa-edit"></i> Editar Módulo';
            ...
            function openStatusModal(id){
                ...
                pendingStatusId = id;
                document.getElementById('statusModalTitle').textContent = isActive?'Desativar Módulo':'Ativar Módulo';
            ...
            function openDeleteModal(id){
                const row = document.getElementById('row-'+id); if (!row) return;
                pendingDeleteId = id;
                document.getElementById('deleteModuleName').textContent = row.querySelector('.cell-actions').dataset.name;
                deleteModal.show();
            }
